...
class Cw6DKoz(Dynamics):
    ...
    _DOMAIN_HI = (800.0, 4000.0, 400.0, 2.0, 2.0, 2.0)
    ...
    def __init__(
        ...
        lo = torch.tensor(self._DOMAIN_LO, dtype=torch.float64)
        hi = torch.tensor(self._DOMAIN_HI, dtype=torch.float64)
        mean = (lo + hi) / 2.0
        var = (hi - lo) / 2.0
    ...
    def state_verification_range(self) -> list[list[float]]:
        return self.state_test_range()
