try:
    import wandb
except ImportError:  # optional; training uses use_wandb=False by default
...
def _safe_reduction(values: torch.Tensor, *, kind: str, fill: float) -> torch.Tensor:
    """``max``/``min`` over ``values``; empty tensor returns scalar ``fill`` on same device."""
    if values.numel() == 0:
        return torch.tensor(fill, device=values.device, dtype=values.dtype)
    if kind == "max":
...
class Experiment(ABC):
    ...
    def validate(self, device, epoch, save_path, x_resolution, y_resolution, z_resolution, time_resolution):
        ...
        # CURRENT: curriculum-aware validation times
        counter_end = max(int(self.dataset.counter_end), 1)
        curriculum_frac = min(1.0, float(self.dataset.counter) / float(counter_end))
        t_val_max = float(
            self.dataset.tMin + curriculum_frac * (self.dataset.tMax - self.dataset.tMin)
        )
        times = torch.linspace(float(self.dataset.tMin), t_val_max, time_resolution)
        # ORIGINAL upstream
        ...
        for i in range(len(times)):
            for j in range(len(zs)):
                ...
                time_lbl = plot_config.get('time_label', 't')
                ax.set_title(
                    '%s = %0.2f (trained ≤%0.2f), %s = %0.2f'
                    % (time_lbl, float(times[i]), t_val_max, plot_config['state_labels'][plot_config['z_axis_idx']], float(zs[j]))
                )
    ...
    def train(
        ...
        # CURRENT: optional checkpoint resume
        start_epoch = int(max(0, start_epoch))
        if resume_checkpoint is not None:
            ...
            if isinstance(ck, dict) and "model" in ck:
                ...
                start_epoch = int(ck.get("epoch", start_epoch))
            else:
                self.model.load_state_dict(ck)
            self.dataset.pretrain = False
            self.dataset.pretrain_counter = self.dataset.pretrain_iters
            self.dataset.counter = min(start_epoch, self.dataset.counter_end)
            print(
                f"Resumed from {ckpt_path} at epoch {start_epoch} "
                f"(curriculum counter={self.dataset.counter}/{self.dataset.counter_end})."
            )

        if start_epoch >= int(epochs):
            print(f"start_epoch {start_epoch} >= epochs {epochs}; nothing to train.")
            return
        ...
        with tqdm(total=len(train_dataloader) * (int(epochs) - start_epoch)) as pbar:
            train_losses = []
            last_CSL_epoch = start_epoch - 1
            for epoch in range(start_epoch, epochs):
                ...
                if use_CSL and not self.dataset.pretrain and (epoch-last_CSL_epoch) >= epochs_til_CSL:
                    ...
                    if self.dataset.dynamics.loss_type == 'brt_hjivi':
                        SSL_val_losses = loss_fn(CSL_val_states, CSL_val_preds, CSL_val_dvs[..., 0], CSL_val_dvs[..., 1:], CSL_val_boundary_values, CSL_val_dirichlet_masks, CSL_val_results['model_out'])
                    elif self.dataset.dynamics.loss_type == 'brat_hjivi':
                        SSL_val_losses = loss_fn(CSL_val_states, CSL_val_preds, CSL_val_dvs[..., 0], CSL_val_dvs[..., 1:], CSL_val_boundary_values, CSL_val_reach_values, CSL_val_avoid_values, CSL_val_dirichlet_masks, CSL_val_results['model_out'])
                    else:
                        NotImplementedError
                    ...
                    for CSL_epoch in tqdm(range(max_CSL_epochs)):
                        ...
                        for CSL_batch in range(math.ceil(num_CSL_samples/CSL_batch_size)):
                            ...
                            if self.dataset.dynamics.loss_type == 'brt_hjivi':
                                SSL_batch_losses = loss_fn(CSL_batch_states, CSL_batch_preds, CSL_batch_dvs[..., 0], CSL_batch_dvs[..., 1:], CSL_batch_boundary_values, CSL_batch_dirichlet_masks, CSL_batch_results['model_out'])
                            elif self.dataset.dynamics.loss_type == 'brat_hjivi':
                                SSL_batch_losses = loss_fn(CSL_batch_states, CSL_batch_preds, CSL_batch_dvs[..., 0], CSL_batch_dvs[..., 1:], CSL_batch_boundary_values, CSL_batch_reach_values, CSL_batch_avoid_values, CSL_batch_dirichlet_masks, CSL_batch_results['model_out'])
                            else:
                                NotImplementedError
                        ...
                        if self.dataset.dynamics.set_mode == 'reach':
                            ...
                            CSL_train_batch_recovered_safe_set_frac = torch.sum(CSL_batch_preds < _safe_reduction(CSL_batch_preds[CSL_batch_costs.to(device) > 0], kind="min", fill=float("inf"))) / len(CSL_batch_preds)
                            CSL_val_theoretically_recoverable_safe_set_frac = torch.sum(CSL_val_costs.to(device) < 0) / len(CSL_val_preds)
                            CSL_val_recovered_safe_set_frac = torch.sum(CSL_val_preds < _safe_reduction(CSL_val_preds[CSL_val_costs.to(device) > 0], kind="min", fill=float("inf"))) / len(CSL_val_preds)
                            CSL_val_tMax_theoretically_recoverable_safe_set_frac = torch.sum(CSL_val_tMax_costs.to(device) < 0) / len(CSL_val_tMax_preds)
                            CSL_val_tMax_recovered_safe_set_frac = torch.sum(CSL_val_tMax_preds < _safe_reduction(CSL_val_tMax_preds[CSL_val_tMax_costs.to(device) > 0], kind="min", fill=float("inf"))) / len(CSL_val_tMax_preds)
                        elif self.dataset.dynamics.set_mode == 'avoid':
                            CSL_train_batch_theoretically_recoverable_safe_set_frac = torch.sum(CSL_batch_costs.to(device) > 0) / len(CSL_batch_preds)
                            CSL_train_batch_recovered_safe_set_frac = torch.sum(CSL_batch_preds > _safe_reduction(CSL_batch_preds[CSL_batch_costs.to(device) < 0], kind="max", fill=float("-inf"))) / len(CSL_batch_preds)
                            CSL_val_theoretically_recoverable_safe_set_frac = torch.sum(CSL_val_costs.to(device) > 0) / len(CSL_val_preds)
                            CSL_val_recovered_safe_set_frac = torch.sum(CSL_val_preds > _safe_reduction(CSL_val_preds[CSL_val_costs.to(device) < 0], kind="max", fill=float("-inf"))) / len(CSL_val_preds)
                            CSL_val_tMax_theoretically_recoverable_safe_set_frac = torch.sum(CSL_val_tMax_costs.to(device) > 0) / len(CSL_val_tMax_preds)
                            CSL_val_tMax_recovered_safe_set_frac = torch.sum(CSL_val_tMax_preds > _safe_reduction(CSL_val_tMax_preds[CSL_val_tMax_costs.to(device) < 0], kind="max", fill=float("-inf"))) / len(CSL_val_tMax_preds)
                        else:
                            raise NotImplementedError
                        if self.use_wandb and wandb is not None:
                            wandb.log({
