...
class ReachabilityDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.pretrain:
            ...
            # CURRENT: curriculum in physical/nondim tau
            tau_span = (self.tMax - self.tMin) * (self.counter / self.counter_end)
            times = self.tMin + torch.zeros(self.numpoints, 1).uniform_(0, tau_span)
            # ORIGINAL upstream
        ...
        # CURRENT: physical coords to coord_to_input
        state_phys = (
            model_states * self.dynamics.state_var.to(device=model_states.device)
            + self.dynamics.state_mean.to(device=model_states.device)
        )
        coords = torch.cat((times, state_phys), dim=1)
        if self.dynamics.input_dim > self.dynamics.state_dim + 1:
            coords = torch.cat(
                (coords, torch.zeros(self.numpoints, self.dynamics.input_dim - self.dynamics.state_dim - 1)),
                dim=1,
            )
        model_coords = self.dynamics.coord_to_input(coords)

        boundary_values = self.dynamics.boundary_fn(state_phys)
        if self.dynamics.loss_type == 'brat_hjivi':
            reach_values = self.dynamics.reach_fn(state_phys)
            avoid_values = self.dynamics.avoid_fn(state_phys)
        ...
        if self.pretrain:
            ...
            dirichlet_masks = (coords[:, 0] == self.tMin)  # CURRENT: physical tau
            # dirichlet_masks = (model_coords[:, 0] == self.tMin)  # ORIGINAL upstream
